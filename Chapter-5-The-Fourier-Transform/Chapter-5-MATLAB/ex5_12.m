% 
% Example 5.12 --- Magnitude and phase spectra
%
clear all;clf
syms t w
...
if ind==1
    ...
    figure(1)
    subplot(221)
    fplot(x1,[-1,5]);grid;axis([-1 5 -0.2 1.2]); title(' '); 
    ylabel('x_1(t)'); xlabel('t (sec)')
    subplot(223)
    fplot(sqrt((real(X1))^2+(imag(X1))^2),[-30,30]); grid; axis([-30 30 -0.2 1.2]);
    xlabel('\Omega'); ylabel('|X_1(\Omega)|'); title(' ')
    subplot(224)
    fplot(imag(log(X1)),[-30,30]); grid; title(' ');
    ...
    figure(1)
    subplot(221)
    fplot(x2,[-1,5]);grid;axis([-1 5 -0.2 1.2]); title(' '); 
    ylabel('x_2(t)'); xlabel('t (sec)')
    subplot(223)
    fplot(X2m,[-30,30]); grid; axis([-30 30 -0.2 1.2]);
    xlabel('\Omega'); ylabel('|X_2(\Omega)|'); title(' ')
    subplot(224)
    fplot(X2a,[-30,30]); grid; title(' ');
