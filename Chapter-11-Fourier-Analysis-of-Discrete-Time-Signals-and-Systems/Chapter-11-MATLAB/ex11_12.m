%
% Example 11.12---Phase of sinusoid in noise
%
clear all; clf
n=0:99;
i=input('sine (1), cos (2)')
if i==1,
x=sin(pi*n/2)+0.1*randn(1,100); % sine plus noise
else
x=cos(pi*n/2)+0.1*randn(1,100);% cosine plus noise     
end
...
ylabel('\theta (rad)');
subplot(313)
plot(w1,theta1);grid;axis([min(w1) max(w1) 1.1*min(theta) 1.1*max(theta)]);
